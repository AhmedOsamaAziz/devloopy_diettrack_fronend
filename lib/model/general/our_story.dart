class OurStoryModel {
  final String image;
  final String title;
  final String subTitle;
  final String date;

  OurStoryModel(
      {required this.image,
      required this.title,
      required this.subTitle,
      required this.date});

  static List<OurStoryModel> ourStories = [
    OurStoryModel(
      image: 'assets/images/about1.png',
      title: 'Inspiring Transformations Story',
      subTitle:
          'Nutritionist continues to empower individuals to transform their lives through personalized nutrition coaching. With an expanding client base and a growing team of experts, we remain committed to our goal of helping people lose weight, improve their health, and lead happier, more fulfilling lives. Our journey of inspiring transformations continues, one client at a time.',
      date: 'July 1, 2025',
    ),
    OurStoryModel(
      image: 'assets/images/about2.png',
      title: 'Inspiring Transformations Story',
      subTitle:
          'Nutritionist continues to empower individuals to transform their lives through personalized nutrition coaching. With an expanding client base and a growing team of experts, we remain committed to our goal of helping people lose weight, improve their health, and lead happier, more fulfilling lives. Our journey of inspiring transformations continues, one client at a time.',
      date: 'July 1, 2025',
    ),
    OurStoryModel(
      image: 'assets/images/about1.png',
      title: 'Inspiring Transformations Story',
      subTitle:
          'Nutritionist continues to empower individuals to transform their lives through personalized nutrition coaching. With an expanding client base and a growing team of experts, we remain committed to our goal of helping people lose weight, improve their health, and lead happier, more fulfilling lives. Our journey of inspiring transformations continues, one client at a time.',
      date: 'July 1, 2025',
    ),
    OurStoryModel(
        image: 'assets/images/about3.png',
        title: 'Inspiring Transformations Story',
        subTitle:
            'Nutritionist continues to empower individuals to transform their lives through personalized nutrition coaching. With an expanding client base and a growing team of experts, we remain committed to our goal of helping people lose weight, improve their health, and lead happier, more fulfilling lives. Our journey of inspiring transformations continues, one client at a time.',
        date: 'July 1, 2025'),
    OurStoryModel(
      image: 'assets/images/about1.png',
      title: 'Inspiring Transformations Story',
      subTitle:
          'Last Nutritionist continues to empower individuals to transform their lives through personalized nutrition coaching. With an expanding client base and a growing team of experts, we remain committed to our goal of helping people lose weight, improve their health, and lead happier, more fulfilling lives. Our journey of inspiring transformations continues, one client at a time.',
      date: 'July 1, 2025',
    ),
  ];
}
